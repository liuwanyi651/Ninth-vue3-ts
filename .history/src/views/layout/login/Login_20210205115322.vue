            ...
            <el-form :model="ruleForm1" :rules="rules1" ref="form1" class="demo-ruleForm">
              ...
              <el-form-item prop="captcha">
                <el-input v-model="ruleForm1.captcha" placeholder="验证码">
                  <template #append>
                    <el-button @click="code">发送验证码</el-button>
                  </template>
                </el-input>
              </el-form-item>
              <!--昵称-->
...
export default defineComponent({
  ...
  setup() {
    ...
    let validator = (rule: any, value: any, callback: any):void => {
      ...
      if (!value) {
        return callback(Error("手机号码不能为空"));
      } else {
        if (
          ...
          return callback(Error("手机号码格式错误"));
        }
        callback();
      }
    };
    // 核对两次密码表单验证
    let validatora = (rule: any, value: any, callback: any):void => {
      if (!value) {
        return callback(Error("确认密码不能为空"));
      } else {
        if (ruleForm1.value.password.length < 6) {
          return callback(Error("确认密码不能少于6位"));
        } else if (ruleForm1.value.password !== ruleForm1.value.checkPass) {
          return callback(Error("两次密码输入不一致"));
        }
        callback();
      }
    };
        ...
        if (valid) {
          ...
          api
            .register({
              ...
              nickname: ruleForm1.value.nickname,
              captcha: ruleForm1.value.captcha,
            })
            .then((res: any) => {
              if(res){
                localStorage.setItem("token", res.token);
    ...
    // 验证码
    const code = (): void =>{
      if(ruleForm1.value.username.length>0){
        api.code({
          tel:ruleForm1.value.username
        }).then((res:any)=>{
          console.log(res)
        })
        ElMessage.success("模拟手机验证码:000000");
      }else{
